...
class MASTERModel(Model):
    def __init__(self, d_feat: int = 158, d_model: int = 256, t_nhead: int = 4, s_nhead: int = 2, gate_input_start_index=158, gate_input_end_index=221,
            T_dropout_rate=0.5, S_dropout_rate=0.5, beta=None, n_epochs = 40, lr = 8e-6, GPU=0, seed=0, train_stop_loss_thred=None, save_path = 'model/', save_prefix= '',
            load_model_path=None, 
            out_pred_path=None,
            benchmark = 'SH000300', market = 'csi300', only_backtest = False):
        ...
        self.out_pred_path = out_pred_path
    ...
    def fit(self, dataset: DatasetH):
        if self.load_model_path is not None:
            ...
            if os.path.exists(self.load_model_path):
                print(f"[Info] Loading pre-trained model from: {self.load_model_path}")
                # 根据你底层模型的具体实现，通常是 load_state_dict
                self.model.load_state_dict(torch.load(self.load_model_path, map_location=self.device))
                self.fitted = True  # 标记模型为已训练状态，防止 predict 报错
                print("[Info] Skip training phase.")
                return
            else:
                print(f"[Warning] Pre-trained model {self.load_model_path} not found! Fallback to training.")
    ...
    def predict(self, dataset: DatasetH, use_pretrained = True):
        ...
        # rec = self.backtest()
        if getattr(self, 'out_pred_path', None) is not None:
            import os
            # 确保保存的文件夹存在
            os.makedirs(os.path.dirname(os.path.abspath(self.out_pred_path)), exist_ok=True)
            # 支持保存为 CSV 或 PKL
            if self.out_pred_path.endswith('.csv'):
                pred_all.to_csv(self.out_pred_path, header=['score'])
            elif self.out_pred_path.endswith('.pkl'):
                pred_all.to_pickle(self.out_pred_path)
            
            print(f"[Success] Prediction results saved to: {self.out_pred_path}")
