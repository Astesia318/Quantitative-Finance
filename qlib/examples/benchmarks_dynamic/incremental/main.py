...
class Incremental:
    ...
    def __init__(
            self,
            data_dir="cn_data",
            root_path='/data/lyc/quant/qlib_bin',
            market="csi300",
            horizon=1,
            alpha=360,
            step=20,
        ...
        if data_dir == "cn_data":
            GetData().qlib_data(target_dir=root_path, exists_skip=True)
            auto_init()
        else:
            qlib.init(
                provider_uri=root_path, region="us" if self.data_dir == "us_data" else "cn",
            )
        self.step = step
    ...
    def dump_data(self):
        ...
        if hasattr(data, 'index'):
            ...
            # TSDatasetH 返回的是 TSDataSampler，没有 index 属性
            # 我们通过底层的 handler 提取对应 segment 的 DataFrame 的 index
            train_seg = t["dataset"]["kwargs"]["segments"]["train"]
            data_index = ds.handler.fetch(selector=train_seg, data_key=DataHandlerLP.DK_L).index
            
            # 动态挂载 index 属性，防止后续其他方法隐式调用 data.index 报错
            data.index = data_index 
            
        assert ta.align_seg(t["dataset"]["kwargs"]["segments"]["train"])[0] == data_index[0][0]
